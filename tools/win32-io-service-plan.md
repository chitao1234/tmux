...
- if the final partial cannot be written, the backend reports
  ...
  drained.
- the retained partial is intentionally not counted as normal writer-buffered
  data because no further write can drain it by itself. It is resolved by the
  close path, which preserves existing ACK-after-message-boundary behavior
  without stalling close barriers.
