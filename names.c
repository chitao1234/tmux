...
static void	 name_time_callback(tmux_event_fd, short, void *);
static int	 name_time_expired(struct window *, struct timeval *);

static char	*format_window_name(struct window *);

static void
name_time_callback(__unused tmux_event_fd fd, __unused short events, void *arg)
{
	struct window	*w = arg;
