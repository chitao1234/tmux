...
client_connect(struct event_base *base, struct ipc_endpoint *endpoint,
    uint64_t flags)
{
	return (ipc_client_connect_or_start(base, client_proc, endpoint, flags));
}
...
client_win32_input_start(void)
{
	...
	client_win32_input = win32_io_reader_new_console(hin,
...
client_win32_output_start(void)
{
	...
	hout = GetStdHandle(STD_OUTPUT_HANDLE);
	client_win32_output = win32_io_writer_new_console_borrowed(&hout,
